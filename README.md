...
- 支持 **多轮工具调用**（最多 10 轮），AI 会根据上一步结果自动决定下一步
...
│  │    TypeScript 5           │   │                           │  │
│  │                           │   │  commands/  ← Controller  │  │
│  │  components/              │   │   ai_cmd    file_cmd      │  │
│  │   AiChatWindow            │   │   config_cmd  network_cmd │  │
│  │   ChatWindow  ChatInput   │   │                           │  │
│  │   MessageBubble           │   │  services/  ← Service     │  │
│  │   ImagePreview            │   │   ai/   chat_service      │  │
│  │   MarkdownRenderer        │   │         tool_registry     │  │
│  │   LoginScreen  UserList   │   │         utility_tools     │  │
│  │   TransferIndicator       │   │   web/  scraper / search  │  │
│  │                           │   │   file/ download / tools  │  │
│  │  hooks/                   │   │   mcp_server              │  │
│  │   useChat  useAiChat      │   │   network_service         │  │
│  │   useLocalStorage         │   │                           │  │
│  │   useTransfers            │   │  models/    ← Model       │  │
│  │                           │   │  server/    ← Infra       │  │
│  │  utils/                   │   │  utils/     ← Utils       │  │
│  │   aiHelper  avatar        │   │  config.rs  ← Config      │  │
│  │   dom       fileIcon      │   │                           │  │
│  │   format    storage       │   └───────────────────────────┘  │
│  │   tauri                   │                                  │
│  │                           │                                  │
...
（Java 解耦思想，单向依赖，各层职责清晰）

| 层级 | 目录 | 职责 |
|------|------|------|
| Controller | `commands/` | Tauri IPC 命令入口，纯委托调用，不含业务逻辑 |
| Service | `services/ai/` | AI 对话、工具注册与多轮调度 |
| | `services/web/` | 网页抓取、搜索引擎集成 |
| | `services/file/` | 文件下载、文件系统 CRUD |
| | `services/mcp_server.rs` | MCP JSON-RPC 2.0 协议服务 |
| | `services/network_service.rs` | 网络接口发现、主机名获取 |
| Model | `models/` | 跨层数据结构定义 |
| Infra | `server/` | HTTP / WebSocket 路由与处理器 |
| Config | `config.rs` + `lanchat.config.json` | 统一配置中心，前后端共享 |

### 前端工程规范

| 层级 | 目录 | 职责 |
|------|------|------|
| 视图 | `components/` | 纯 UI 组件，仅接收 props，不直接访问外部 API |
| 状态 | `hooks/` | 封装副作用与跨组件状态，组件通过 Hook 解耦 |
| 工具 | `utils/` | 纯函数，无副作用，可在任意位置安全引用 |
| 类型 | `types.ts` | 前后端共享的 TypeScript 类型定义（单一来源） |
| 配置 | `config.ts` | 运行时配置加载与缓存（从 Rust 后端获取） |

**`utils/` 各模块说明：**

| 文件 | 说明 |
|------|------|
| `aiHelper.ts` | AI 工具推断（`detectToolHint`）、工具结果解析（`extractSources` / `extractFileCards`） |
| `avatar.ts` | 根据昵称哈希生成确定性头像颜色类名 |
| `dom.ts` | DOM 操作工具（`autoResizeTextarea` — textarea 自动高度） |
| `fileIcon.ts` | 文件扩展名 → FileCard 图标类型映射（单一来源） |
| `format.ts` | `formatFileSize` 字节格式化、`formatError` 错误信息转字符串 |
| `storage.ts` | 聊天消息本地持久化（`loadMessages` / `saveMessages` / `getStableClientId`） |
| `tauri.ts` | 安全封装 Tauri `invoke`，非 Tauri 环境下静默返回 null |
...
```json
{
  ...
  "ai":        { "api_url": "https://api.deepseek.com/chat/completions",
                 "model": "deepseek-chat", "max_tokens": 4000, "max_tool_rounds": 10 },
