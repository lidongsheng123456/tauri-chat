...
use crate::models::ai::{BrowseArgs, FetchRawArgs};
    ...
    match req.method.as_str() {
        ...
        "tools/list" => {
            JsonRpcResponse::success(req.id, serde_json::json!({
                "tools": [
                    {
                        "name": "browse_website",
                        "description": "抓取并解析网页内容，返回网页标题、正文文本和链接列表。用于获取网站的完整资料。",
                        "inputSchema": {
                            "type": "object",
                            "properties": {
                                "url": {
                    ...
                    {
                        "name": "fetch_url_raw",
                        "description": "获取 URL 的原始文本内容，适用于 API 接口或 JSON 数据。",
                        "inputSchema": {
                            "type": "object",
                            "properties": {
                                "url": {
        ...
        "tools/call" => {
            ...
            let result_text = match tool_name {
                "browse_website" => {
                    match serde_json::from_value::<BrowseArgs>(arguments) {
                        Ok(args) => web_scraper::browse_website(&args.url).await,
                        Err(e) => format!("参数解析失败: {}", e),
                    }
                }
                "fetch_url_raw" => {
                    match serde_json::from_value::<FetchRawArgs>(arguments) {
            ...
            let is_error = result_text.starts_with("抓取网页失败") ||
                           result_text.starts_with("获取 URL 内容失败") ||
                           result_text.starts_with("未知工具") ||
